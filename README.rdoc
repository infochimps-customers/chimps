...
- {Catalog}[http://www.infochimps.com/catalog-api] List, Show, Create, Update, and Destroy Catalogs
- {Query}[http://www.infochimps.com/apis] Query a particular Catalog

You will need an API key in order to query specific catalogs, and to create/destroy catalogs.
...
You'll need an API key from Infochimps before you
can start adding or modifying datasets.  {Sign up
for an Infochimps account}[http://www.infochimps.com/signup] and get
your {API key}[http://www.infochimps.com/me].
...
configuration file looks like
    ...
    username: banana
    apikey: banananananananananananananana
...
Infochimps will require your API key.  You can see
a full list of the Catalog API endpoints available and whether or not
requests to each need an API key at
...
The authentication mechanism uses the API key (which is
